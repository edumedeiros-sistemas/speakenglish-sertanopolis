// SISTEMA DE SINCRONIZAÇÃO FINAL - SPEAKENGLISH v3.0.0
// Versão final corrigida com tratamento robusto de erros

console.log('🔄 Carregando Sistema de Sincronização Final...');

class SystemSyncFinal {
    ...
    static async initialize() {
        console.log('🚀 Inicializando Sistema de Sincronização Final...');
        ...
        try {
            ...
            if (supabaseReady) {
                ...
                console.log('✅ Sistema de Sincronização Final inicializado!');
            } else {
    ...
    static async uploadTableDataSafe(table, data, dataType) {
        ...
        try {
            ...
            const batches = this.createBatches(data, 10); // Batches menores para evitar erros
            ...
            for (let i = 0; i < batches.length; i++) {
                ...
                try {
                    ...
                    // Tentar insert primeiro, depois upsert se falhar
                    ...
                    if (error) {
                        ...
                        // Se erro de coluna faltante, tentar sem a coluna problemática
    ...
    static simplifyDataForUpload(data, dataType, errorMessage) {
        // Identificar qual coluna está causando problema
    ...
    static async mergeDataSafe() {
        ...
        // Por simplicidade, priorizar dados locais
        // Em versões futuras, comparar timestamps
        await this.uploadLocalDataSafe();
    }
...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(async () => {
        await SystemSyncFinal.initialize();
    }, 3000);
});

// SUBSTITUIR SYSTEMSYNC ORIGINAL
window.SystemSync = SystemSyncFinal;

console.log('🔄 Sistema de Sincronização Final carregado!'); 